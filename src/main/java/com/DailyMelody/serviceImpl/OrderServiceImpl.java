...
import com.DailyMelody.configure.AlipayTools;
import com.DailyMelody.enums.OrderStatusEnum;
import com.DailyMelody.po.*;
import com.DailyMelody.repository.*;
import com.DailyMelody.exception.DailyMelodyException;
import com.DailyMelody.po.*;
import com.DailyMelody.repository.*;
import com.DailyMelody.service.OrderService;
import com.DailyMelody.serviceImpl.strategy.Context;
...
public class OrderServiceImpl implements OrderService {
    ...
    public String pay(Integer orderId,Integer couponId) {
        ...
        if(order == null){
            throw DailyMelodyException.orderNotExists();
        }
        if(order.getStatus() != OrderStatusEnum.UNPAID){
            throw DailyMelodyException.orderStatusError();
        }
        ...
        if (couponId!=0){
            ...
            couponGroup.getStoreId()>0 && !couponGroup.getStoreId().equals(product.getStoreId())){
                throw DailyMelodyException.couponNotAllowed();
            }
    ...
    public Boolean deliver(Integer orderId) {
        ...
        if (product.getStock()<order.getAmount()){
            throw DailyMelodyException.stockNotEnough();
        }
